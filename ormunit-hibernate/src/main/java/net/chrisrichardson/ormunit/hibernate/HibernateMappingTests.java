...
public abstract class HibernateMappingTests extends HibernateORMTestCase {
	...
	protected void assertClassMapping(Class type, String tableName) {
		...
		if (!tableName.equals(classMapping.getTable().getName()))
	...
	protected void assertAllFieldsMapped() throws MappingException {
		assertAllFieldsMappedExcept(Collections.EMPTY_SET);
	}
	...
	protected void assertAllFieldsMappedExcept(Set fieldNames)
		...
		assertNoOtherUnmappedFields(type, getRoots(fieldNames));
		assertFieldsExists(type, getRoots(fieldNames));
	}
	...
	private void assertAllFieldsMapped(Component cv, Set fieldsToIgnore) {
		...
		for (Iterator it = HibernateAssertUtil.getPersistableFields(
				cv.getComponentClass()).iterator(); it.hasNext();) {
	...
	private void assertFieldsExists(Class type, Set fieldNames) {
		Set persistableFields = HibernateAssertUtil.getPersistableFields(type);
	...
	private void assertNoOtherUnmappedFields(Class type, Set nonPersistentFields)
			throws MappingException {
		...
		for (Iterator it = HibernateAssertUtil.getPersistableFields(type)
				.iterator(); it.hasNext();) {
			String fieldName = (String) it.next();
